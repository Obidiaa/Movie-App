...
import com.obidia.movieapp.data.remote.response.ItemSearch
...
@Singleton
class GetSearchPagingSource @Inject constructor(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ItemModel> {
        ...
        return try {
            val response = remoteDataSource.getSearch(page = page, query = query)
            val list = ItemSearch.transform(response.results)
            LoadResult.Page(
                data = list,
